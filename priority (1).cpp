...
int main(){
	...
	cin>>n;
	process p[n];
	cout<<"Enter the arrival, burst time and the priority: ";
	for(int i = 0;i < n;i++){
		int x,y,z;
		cin>>x>>y>>z;
		p[i].id = i;
	...
	cout<<"id\t\tpri\t\tat\t\tbt\t\tst\t\tct\t\twt\t\ttt\t\tntt\n";
	int twt = 0, tt = 0;
	for(int i = 0;i < n;i++){
		cout<<p[i].id+1<<"\t\t"<<p[i].pri<<"\t\t"<<p[i].at<<"\t\t"<<p[i].bt<<"\t\t"<<p[i].ct-p[i].bt<<"\t\t"<<p[i].ct<<"\t\t"<<p[i].ct-p[i].at-p[i].bt<<"\t\t"<<p[i].ct-p[i].at<<"\t\t"<<(p[i].ct-p[i].at) / (float) p[i].bt<<"\t\t"<<endl;
